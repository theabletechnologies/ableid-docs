...
>    - Получение данных о работе резидента
...
| `/check/get_work`                          | `WORK_WEBHOOK_URL`         | Данные о текущем месте работы                              |
...
### `Хук получения данных о работе`

**Куда приходит:** На статический URL для данных о работе.
      ...
      "positions": [
        {
          ...
          "orgTin": "123456789",
          "position": "DASTURCHI",
          "positionRu": "ПРОГРАММИСТ",
          "depName": "AXBOROT TEXNOLOGIYALARI BO'LIMI",
          "depNameRu": "ОТДЕЛ ИНФОРМАЦИОННЫХ ТЕХНОЛОГИЙ",
          ...
          "egovCategory": "П",
          "egovRange": "0",
          "egovMinEducation": "ССПО",
          "egovSpecialityCode": "3150803",
          "egovType": "s",
          "egovName": "Дастурловчи"
        }
      ]
    }
  },
...
| Поле                 | Тип              | Описание                                                                                                                               |
|----------------------|------------------|----------------------------------------------------------------------------------------------------------------------------------------|
| `orgName`            | `string \| null` | Название организации                                                                                                                   |
| `orgTin`             | `string \| null` | ИНН организации                                                                                                                        |
| `position`           | `string \| null` | Должность                                                                                                                              |
| `positionRu`         | `string \| null` | Должность на русском языке                                                                                                             |
| `depName`            | `string \| null` | Название отдела                                                                                                                        |
| `depNameRu`          | `string \| null` | Название отдела на русском языке                                                                                                       |
| `beginDate`          | `string \| null` | Дата поступления на работу (формат `YYYY-MM-DD`)                                                                                       |
| `rate`               | `string \| null` | Ставка (диапазон `0.01`–`1.99`)                                                                                                        |
| `docBeginNum`        | `string \| null` | Номер приказа о трудоустройстве                                                                                                        |
| `kodpPn`             | `string \| null` | Классификатор основных должностей служащих и профессий рабочих ([справочник](https://cs.egov.uz/#/tableData/61a5ca17b83d3e9e6a91c944)) |
| `egovNskzCode`       | `string \| null` | Код НСКЗ — Национального стандартного классификатора занятий                                                                           |
| `egovClass`          | `string \| null` | Класс должности по классификатору                                                                                                      |
| `egovCategory`       | `string \| null` | Категория персонала (например `П` — профессионал)                                                                                      |
| `egovRange`          | `string \| null` | Тарифный разряд                                                                                                                        |
| `egovMinEducation`   | `string \| null` | Минимальный уровень образования (например `ССПО`)                                                                                      |
| `egovSpecialityCode` | `string \| null` | Код специальности                                                                                                                      |
| `egovType`           | `string \| null` | Тип записи в классификаторе (`i` — рабочие профессии)                                                                                  |
| `egovName`           | `string \| null` | Наименование должности из классификатора (узбекский, кириллица)                                                                        |

> **Примечание:** Массив `positions` может содержать несколько объектов, если у пользователя несколько действующих мест работы.
...
|------------------------|------------------|---------------------------------------------------------------------------------------|
| `document`             | `string`         | Серия и номер паспорта                                                                |
| `reg_date`             | `string`         | Дата и время въезда                                                                   |
