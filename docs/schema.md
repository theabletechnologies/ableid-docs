...
2. **AbleID → Партнерское приложение**: выполняется редирект на соответствующую страницу:
   ...
   - В случае WEB-интеграции AbleID отправляет postMessage:
...
| `message`    | `string`  | Человекочитаемое описание результата (например `"Успешно"` или `"Лицо не прошло проверку"`) |
...
| `message`    | `string`  | Человекочитаемое описание результата (например `"Успешно"`) |
...
| `message`    | `string`  | Человекочитаемое описание ошибки (например `"Лицо не прошло проверку"`)  |
| `redirect`   | `string`  | URL страницы, на которую нужно перенаправить пользователя                |
...
### Обработка сообщения (родительское окно)
