...
- **Ошибка при попытке использования:** При обращении к истекшей сессии возвращается ошибка:
  ```json
  {
    ...
    "message": "Время сессии истекло" // на языке сессии
  }
  ```
...
2. **AbleID → Партнерское приложение**: выполняется редирект на соответствующую страницу:
   ...
   - Перед редиректом AbleID отправляет событие в host bridge. Схема payload одинакова на всех платформах:
     - **`redirect`**: `type`, `success`, `statusCode`, `message`, `url`
     - **`complete`**: `type`, `success`, `statusCode`, `message`, `redirect`, `data`
     - **`incomplete`**: `type`, `success`, `statusCode`, `message`, `redirect`, `data`
...
## Пример обработки host bridge

### Каналы доставки host bridge

Выбор транспорта выполняется автоматически в следующем порядке: **iOS → Android → WEB fallback**.

**Фиксированное имя нативного bridge:** `ableIdBridge`.

- В **iOS WKWebView** host должен зарегистрировать `WKScriptMessageHandler` с именем `ableIdBridge`
- В **Android WebView** host должен инжектить JavaScript interface в `WebView` под именем `ableIdBridge`
- В **WEB fallback** отдельное имя bridge не используется, так как доставка идёт через `window.parent.postMessage`

| Тип интеграции      | Канал доставки                                                    | Что получает host            |
|---------------------|-------------------------------------------------------------------|------------------------------|
| **WEB**             | `window.parent.postMessage(payload, targetOrigin)`                | JavaScript-объект `payload`  |
| **iOS WKWebView**   | `window.webkit.messageHandlers.ableIdBridge.postMessage(payload)` | JavaScript-объект `payload`  |
| **Android WebView** | `window.ableIdBridge.postMessage(JSON.stringify(payload))`        | JSON-строку с тем же payload |

---

### Описание полей

#### Сообщение type: `redirect`

| Поле         | Тип       | Описание                                                                            |
|--------------|-----------|-------------------------------------------------------------------------------------|
| `type`       | `string`  | Всегда `"redirect"`. Означает, что нужно перенаправить пользователя                 |
| `success`    | `boolean` | `true` — процесс завершён успешно, `false` — произошла ошибка                       |
| `statusCode` | `number`  | HTTP-код результата. `200` — успех. Другие значения — код ошибки (например `10203`) |
| `message`    | `string`  | Описание результата на языке сессии (`ru`, `uz`, `oz`, `en`)                        |
| `url`        | `string`  | URL страницы, на которую нужно перенаправить пользователя                           |

#### Сообщение type: `complete`

| Поле         | Тип       | Описание                                                     |
|--------------|-----------|--------------------------------------------------------------|
| `type`       | `string`  | Всегда `"complete"`. Означает успешное завершение процесса   |
| `success`    | `boolean` | Всегда `true`                                                |
| `statusCode` | `number`  | Всегда `200`                                                 |
| `message`    | `string`  | Описание результата на языке сессии (`ru`, `uz`, `oz`, `en`) |
| `redirect`   | `string`  | URL страницы, на которую нужно перенаправить пользователя    |
| `data`       | `object`  | Дополнительные данные о результате процесса                  |

#### Сообщение type: `incomplete`

| Поле         | Тип       | Описание                                                                 |
|--------------|-----------|--------------------------------------------------------------------------|
| `type`       | `string`  | Всегда `"incomplete"`. Означает, что процесс завершился с ошибкой        |
| `success`    | `boolean` | Всегда `false`                                                           |
| `statusCode` | `number`  | Код ошибки (например `10203` — лицо не прошло проверку)                  |
| `message`    | `string`  | Описание ошибки на языке сессии (`ru`, `uz`, `oz`, `en`)                 |
| `redirect`   | `string`  | URL страницы, на которую нужно перенаправить пользователя                |
| `data`       | `object`  | Дополнительные данные об ошибке                                          |

---

### Redirect — успех (type: `redirect`)
...
### WEB: обработка сообщения в родительском окне

```javascript
window.addEventListener("message", function (event) {
  const { type, success, statusCode, message, redirect, data, url } = event.data;
