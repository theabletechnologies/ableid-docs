...
> **Для получения информации о получении определенных данных (паспорт, прописка, фото, данные нерезидента) через API и настройке Webhooks перейдите в раздел [Webhooks](/docs/hooks).**
