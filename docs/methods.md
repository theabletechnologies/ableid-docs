...
| `transactionId` | `string`                        | Уникальный ID транзакции клиента | Да           | ""                               |
...
**Описание:** Запрос на получение данных о текущем месте работы
...
#### `Успешный ответ: Придет на хук ранее прописанный для получения данных о работе`
...
Параметр `lang` определяет язык интерфейса пользователя и сообщений в ответах API.
...
| `uz` | Узбекский (латиница)  | O'zbek tili       |
| `oz` | Узбекский (кириллица) | Ўзбек тили        |

### Что зависит от `lang`

- **Интерфейс WebView** — все тексты, инструкции и сообщения в окне идентификации/верификации отображаются на выбранном языке (поддерживаются все 4 языка: `ru`, `en`, `uz`, `oz`)
- **Сообщения в ответах API** — поле `message` в ответах (успех/ошибка) локализуется
- **Данные в вебхуках** — персональные данные из государственных сервисов возвращаются только на 3 языках: `ru`, `uz`, `oz`. Английский (`en`) для данных не поддерживается
- **Поле `lang` в вебхуках** — передаётся в данных вебхука для информирования клиента о языке данных

### Использование

Параметр `lang` передаётся при создании сессии (методы `start`). Если не указан, используется `ru` по умолчанию.
...
> **Важно:** Язык задаётся один раз при создании сессии и не может быть изменён в процессе.
